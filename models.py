...
class Category:
    ...
    def create(name, slug):
        response = supabase.table('categories').insert({'name': name, 'slug': slug}).execute()
        return response.data[0] if response.data else None
...
class Product:
    ...
    @staticmethod
    def get(id):
        response = supabase.table('products').select("*, categories(name)").eq('id', id).execute()
        if response.data:
            p = response.data[0]
            p['category'] = p['categories'] if 'categories' in p else None
            return p
...
class Cart:
    ...
    def get_user_cart(user_id):
        ...
        # Get items
        items_response = supabase.table('cart_items').select("*, products(*)").eq('cart_id', cart['id']).execute()
    ...
    def add_item(cart_id, product_id, quantity):
        ...
        response = supabase.table('cart_items').select("*").eq('cart_id', cart_id).eq('product_id', product_id).execute()
        
        if response.data:
            # Update quantity
            new_qty = response.data[0]['quantity'] + int(quantity)
            supabase.table('cart_items').update({'quantity': new_qty}).eq('id', response.data[0]['id']).execute()
        else:
            # Insert new item
...
class Order:
    ...
    @staticmethod
    def get_all():
        response = supabase.table('orders').select("*, users(email)").order('created_at', desc=True).execute()
        orders = []
    ...
    @staticmethod
    def get(id):
        response = supabase.table('orders').select("*, users(email)").eq('id', id).execute()
        if not response.data:
            return None
